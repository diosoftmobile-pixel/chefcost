...
router.post('/', requireSubscription, (req, res) => {
  const { name, category, unit, purchase_qty, purchase_price, supplier = '', notes = '', allergens = [] } = req.body;
  ...
  db.prepare(`INSERT INTO ingredients (id,user_id,name,category,unit,purchase_qty,purchase_price,supplier,notes,allergens) VALUES (?,?,?,?,?,?,?,?,?,?)`)
...
router.put('/:id', requireSubscription, (req, res) => {
  ...
  const { name, category, unit, purchase_qty, purchase_price, supplier, notes, allergens = [] } = req.body;
  const allergensJson = JSON.stringify(Array.isArray(allergens) ? allergens : []);
  db.prepare(`UPDATE ingredients SET name=?,category=?,unit=?,purchase_qty=?,purchase_price=?,supplier=?,notes=?,allergens=?,updated_at=datetime('now') WHERE id=?`)
    .run(name, category, unit, purchase_qty, purchase_price, supplier ?? '', notes ?? '', allergensJson, req.params.id);

    ...
    <>
      ...
      <div className="page-content">
        <div className="grid-4">
          <div className="stat-card"><div className="stat-label">Total events</div><div className="stat-value">{events.length}</div><div className="stat-meta">{upcoming} upcoming</div></div>
          <div className="stat-card"><div className="stat-label">Pipeline value</div><div className="stat-value accent">{fmt(totalRevenue)}</div><div className="stat-meta">All active events</div></div>
          <div className="stat-card"><div className="stat-label">Recipes</div><div className="stat-value">{recipes.length}</div><div className="stat-meta">{ingredients.length} ingredients</div></div>
          <div className="stat-card"><div className="stat-label">Approved</div><div className="stat-value green">{approved}</div><div className="stat-meta">Confirmed bookings</div></div>
        </div>
        <div className="grid-2">
          <div className="card">
            <div className="card-header"><i className="ti ti-notebook accent"></i><span className="card-title">Recipe cost ranking</span></div>
            <div className="card-body">
              {sortedRecipes.length === 0 && <div className="empty-state"><i className="ti ti-notebook"></i><p>No recipes yet</p></div>}
              {sortedRecipes.map(r => {
                ...
                return <div key={r.id} style={{ marginBottom: 14 }}>
                  ...
                  <div className="text3" style={{ fontSize: 11, marginTop: 3 }}>{fmt(calcCostPerPortion(r, ingredients))}/portion · {r.portions} portions</div>
                </div>;
              })}
            </div>
          </div>
          <div className="card">
            <div className="card-header"><i className="ti ti-calendar-event accent"></i><span className="card-title">Upcoming events</span></div>
            <div className="card-body">
              {recentEvents.length === 0 && <div className="empty-state"><i className="ti ti-calendar-event"></i><p>No events yet</p></div>}
              {recentEvents.map(e => (
                <div key={e.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
                  <div>
                    ...
                    <div className="text3" style={{ fontSize: 11 }}>{e.event_date} · {e.guest_count} guests</div>
                  </div>
